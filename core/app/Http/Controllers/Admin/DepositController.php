...
class DepositController extends Controller
{
    ...
    {
        if ($scope) {
            $deposits = Deposit::$scope()->with(['doctor', 'gateway']);
        } else {
            $deposits = Deposit::with(['doctor', 'gateway']);
        }

        if ($userId) {
            $deposits = $deposits->where('doctor_id', $userId);
        }
    ...
    {
        $deposit = Deposit::where('id', $id)->with(['doctor', 'gateway'])->firstOrFail();
        // $pageTitle = $deposit->doctor->username . ' requested ' . showAmount($deposit->amount);
        $pageTitle = $deposit->clinic->name . ' requested ' . showAmount($deposit->amount);
