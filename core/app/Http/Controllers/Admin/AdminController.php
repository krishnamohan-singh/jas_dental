...
class AdminController extends Controller
{
    ...
    public function depositAndWithdrawReport(Request $request)
    {
        ...
        $deposits = Deposit::successful()->where('created_at', '>=', Carbon::now()->subYear())
    ...
    public function transactionReport(Request $request)
    {
        ...
        $appointmentCount = Appointment::where('try', Status::YES)->where('booking_date', '>=', Carbon::now()->subDays(30))
            ->whereDate('created_at', '>=', $request->start_date)
            ->whereDate('created_at', '<=', $request->end_date)
            ->selectRaw("SUM(try) as appointmentTry, DATE_FORMAT(booking_date,'{$format}') as date")
            ->orderBy('created_at')
            ->groupBy('date')
            ->get();
        ...
        foreach ($dates as $date) {
            $data[] = [
                'date' => $date,
                'appointments' => @$appointmentCount->where('date', $date)->first()->appointmentTry ?? 0,
            ];
        }

        $data = collect($data);

        // monthly appointment report graph
        $report['date']   = $data->pluck('date');
        $report['data']     = [
            [
                'name' => 'Total Appointments',
                'data' => $data->pluck('appointments')
            ]
        ];
    ...
    {
        $notifications = AdminNotification::orderBy('id', 'desc')->with('doctor')->paginate(getPaginate());
