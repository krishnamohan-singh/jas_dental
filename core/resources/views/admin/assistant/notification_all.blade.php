            ...
            <div class="row  mb-4 justify-content-center">
                <div class="col-sm-7">
                    <div class="row gy-4 justify-content-center">
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-list" :title="$viaText . ' should be sent'" value="{{ @$sessionData['total_assistant'] }}"
                        ...
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fa-solid fa-paper-plane" :title="$viaText . ' has yet to be sent'" :viewMoreIcon=false
                                value="{{ @$sessionData['total_assistant'] - @$sessionData['total_sent'] }}" bg="warning" />
                        </div>
                        <div class="col-sm-6">
                                ...
                                <div class="card-body p-5 text-center">
                                    <div class="coaling-loader flex-column d-flex justify-content-center">
                                        ...
                                        <p class="text--primary">
                                            @lang(' ' . @$sessionData['total_sent'] . ' out of ' . @$sessionData['total_assistant'] . ' ' . $viaName . ' were successfully transmitted')
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToAssistant as $key => $toUser)
                                    ...
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start" value="{{ old('start', @$sessionData['start']) }}"
                                                type="number" placeholder="@lang('Start form assistant id. e.g. 1')" required>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" value="{{ old('batch', @$sessionData['batch']) }}"
                                                    type="number" placeholder="@lang('How many assistant')" required>
    ...
    <script>
        ...
        (function($) {
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                if (methodName == 'SELECTEDASSISTANTS') {
                    $('.input-append').html(`
                    <div class="form-group" id="user_list_wrapper">
                        <label class="required">@lang('Select Assistant')</label>
                        <select name="assistant[]"  class="form-control" id="user_list" required multiple >
            ...
            function fetchUserList() {
                ...
                $('.row #user_list').select2({
                    ajax: {
                        url: "{{ route('admin.assistant.list') }}",
                        type: "get",
                        dataType: 'json',
                        delay: 1000,
                        ...
                        processResults: function(response, params) {
                            ...
                            let data = response.assistants.data;
                            return {
            ...
            function getUserCount(methodName) {
                ...
                if (methodNameUpper == 'SELECTEDASSISTANTS' || methodNameUpper == 'ALLASSISTANTS') {
                    $('.userCount').text(0);
                    $('.userCountText').addClass('d-none');
                    return;
                }
                var route = "{{ route('admin.assistant.segment.count', ':methodName') }}"
